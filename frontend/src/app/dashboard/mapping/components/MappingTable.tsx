      ...
      <table className="w-full text-base">
        ...
        <tbody>
          {columns.map(col => {
            ...
            const isAutoGen = col.COLUMN_NAME === 'LOG_ID' || col.COLUMN_NAME === 'CREATED_AT';

            return (
              <tr key={col.COLUMN_NAME}
                className={`border-b border-border/50 dark:border-border-dark/50 transition-colors
                  ${isAutoGen ? 'bg-muted/30 dark:bg-muted/10' : isMapped ? 'bg-success/5 dark:bg-success/5' : 'hover:bg-surface/50 dark:hover:bg-background-dark/50'}`}>
                ...
                <td className="px-4 py-2">
                  ...
                  {isAutoGen && (
                    <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-muted dark:bg-muted/30 text-muted-foreground">
                      {t('mapping.autoGenerated')}
                    </span>
                  )}
                </td>
                <td className="px-4 py-2">
                ...
                <td className="px-4 py-2">
                  {isAutoGen ? (
                    <span className="text-xs text-muted-foreground italic">{t('mapping.autoGeneratedDesc')}</span>
                  ) : logType ? (
                    <select
